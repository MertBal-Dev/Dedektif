 ...
 * @param imageUrl Gemini/Imagen tarafından üretilen geçici görsel URL'i
 * @param caseId Vaka ID'si (klasörleme için)
 * @param fileName Kaydedilecek dosya adı (viktima, char1, env1 vb.)
...
export async function processAndUploadImage(
  ...
  try {
    ...
    // 1024x1024 sınırını korur, metadata temizler ve %85 kalite ile WebP yapar.
...
export async function uploadAllCaseImages(caseId: string, imageTasks: { url: string; name: string }[]): Promise<Record<string, string>> {
  ...
  // 3'erli gruplar halinde paralel işle (Supabase ve Sharp tarafında aşırı yüklenmemek için)
  const chunkSize = 3;
